  ...
  return (
    <div>
      ...
      <div className="bg-blue-300 col-span-1 rounded-2xl p-4 flex justify-center">
        <img src={doctor.img} alt={doctor.name} className="rounded-lg w-full" />
      </div>

      <div className="md:col-span-3 cursor-pointer mt-3 md:mt-0">
        <div className="border border-gray-500 rounded-xl px-5 py-10">
          <h1 className="md:text-4xl text-2xl font-bold text-gray-600">{doctor.name}</h1>
          <div className="mt-5">
            <p className="text-xl font-medium text-gray-500 pt-3">
              Speciality: <span className="pl-3">{doctor.speciality}</span>
            </p>
            <p className="text-xl font-medium text-gray-500 pt-3">
              Experience: <span className="pl-3">{doctor.experience} years <VerifiedIcon fontSize="small" /></span>
            </p>
          </div>
          <div className="mt-5">
            <p className="text-lg font-semibold pt-2">
              About <span className="pl-1"><InfoIcon fontSize="small" color="grey" /></span>
            </p>
            <p className="pt-4">{doctor.about}</p>
          </div>
          <p className="font-semibold text-lg pt-10">
            Appointment Fee: <span className="pl-1">{doctor.fee} <CurrencyRupeeIcon fontSize="small" /></span>
          </p>
        </div>
      </div>
      ...
      <div className="md:col-span-4 flex flex-col md:flex-row mt-10 bg-white shadow-lg p-6 rounded-lg">
        ...
        <div className="md:w-1/2">
          <h2 className="text-xl font-bold mb-4 text-center">Select an Appointment Date</h2>
          <Calendar
            onChange={setDate}
            value={date}
        ...
        <div className="md:w-1/2 mt-6 md:mt-0 md:ml-10">
          <h2 className="text-xl font-bold mb-4 text-center">Select a Time Slot</h2>
          ...
          {selectedTime && (
            <p className="mt-4 text-lg font-medium text-center">
              Selected Time: <span className="text-green-500">{selectedTime}</span>
            </p>
          )}
        </div>
      </div>
    ...
    <div className=' flex justify-center mt-5 '>
        <button onClick={handleBooking}  className="btn btn-wide bg-blue-500 md:my-5 my-3 md:py-7 hover:bg-blue-700 text-white font-bold ">Book An Appointment</button>
    </div>
    </div>
  );
};
